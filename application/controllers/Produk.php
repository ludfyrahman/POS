...
class Produk extends CI_Controller {
    ...
    public function index(){
		...
		$id = Account_Helper::Get('id_toko');
		if($_SESSION['userlevel'] == 1){
			$data['data'] = $this->db->get("$this->low")->result_array();
		}else{
			$data['data'] = $this->db->query("SELECT p.nama, st.*,p.harga, st.jumlah as stok FROM produk p JOIN stok_toko st ON st.id_produk=p.id WHERE st.id_toko='$id'")->result_array();
		}
	...
	public function store(){
		...
		try{
			...
			[
				...
				'100gr' => $this->input->post('100gr'), 
				'500gr' => $this->input->post('500gr'), 
	...
	public function getHarga($idProduk, $tipe){
